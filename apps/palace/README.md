...
### Schema management (Drizzle + Supabase)

Schema is defined in Drizzle TS files at `src/server/db/schema/`. Migrations are generated by Drizzle Kit and stored in `supabase/migrations/` so Supabase CLI can apply them.

**Day-to-day workflow:**

1. Edit Drizzle schema (add table, column, etc.)
2. Generate a migration:
   ```bash
   bun run db:generate
   ```
3. Apply to local DB (choose one):
   - **Quick sync** (no migration file needed): `bun run db:push:fast`
   - **Full reset** (drops DB, reapplies all migrations + seeds): `bun run db:reset`
   - **Incremental migrate** (applies only pending migrations): `bun run db:migrate`

**All schema commands:**

- `bun run db:generate` — generate a new migration from schema changes
- `bun run db:migrate` — apply pending migrations via Drizzle Kit
...
- `db:reset` and `db:migrate` both apply migrations but use different tracking. After a `db:reset`, use `db:push:fast` or `db:reset` again for further changes (not `db:migrate`).
- Migration files in `supabase/migrations/` are committed to the repo.
- For custom SQL not covered by Drizzle (extensions, functions, triggers), add a [custom migration](https://orm.drizzle.team/docs/kit-custom-migrations) via `drizzle-kit generate --custom`.
